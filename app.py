...
# ─── Session State Init ──────────────────────────────────────────────────────────
# Persist slider values across button presses so quick scenarios actually stick
...
<style>
    ...
    /* ── Scenario buttons — override Streamlit defaults ── */
    /* Base style for all scenario buttons */
    ...
    div[data-testid="stButton"]:nth-of-type(4) > button:hover {
    ...
    div[data-testid="stButton"]:nth-of-type(5) > button:hover {
...
PLOTLY_LAYOUT = dict(
    ...
    hoverlabel=dict(
        bgcolor="#1e2535", font_size=12,
        font_family="DM Mono", bordercolor="#374151",
    ),
)
...
def add_funding_annotation(fig: go.Figure, df) -> go.Figure:
    for _, row in df[df["funding_event"] > 0].iterrows():
        ...
        fig.add_annotation(
            x=idx, y=1, xref="x", yref="paper",
            text="Series A", showarrow=False,
            font=dict(color="#fbbf24", size=10), yanchor="bottom",
        )
    return fig

def runway_insight(runway: float, rev_change: int, burn_change: int) -> str:
    """Generate a dynamic contextual insight based on current scenario."""
    if runway <= 0:
        ...
        return ("danger", f"🚨 Only {runway:.1f} months of runway remaining. The company would need to raise or cut costs urgently — most investors want to see 18+ months before leading a round.")
    elif runway < 12:
        return ("warning", f"⚠️ {runway:.1f} months of runway. This is below the typical 18-month fundraising buffer. Time to start conversations with investors or tighten operations.")
    elif runway < 18:
        return ("warning", f"🟡 {runway:.1f} months of runway. Healthy but worth monitoring. A 10% revenue miss or unexpected hiring could push this into the danger zone.")
    else:
        return ("success", f"✅ {runway:.1f} months of runway. Strong position — enough breathing room to focus on growth rather than survival. Series B conversations can start from a position of strength.")
...
with badge_col:
    if st.session_state.active_scenario:
        st.markdown(
            ...
            f'<span class="active-badge">📊 {st.session_state.active_scenario}</span>'
            f'</div>',
            unsafe_allow_html=True,
        )
...
# Sync slider values back to session state
st.session_state.rev_change = rev_change
st.session_state.burn_change = burn_change

st.markdown(
    '<p style="font-size:12px; color:#6b7280; margin: 14px 0 8px 0;">⚡ Quick Scenarios</p>',
    unsafe_allow_html=True,
)

btn1, btn2, btn3, btn4, btn5, _ = st.columns([1.1, 1.1, 1.1, 1, 0.9, 1.8])
...
with btn1:
    if st.button("🚀 Strong Growth", use_container_width=True):
        ...
        st.rerun()
with btn2:
    if st.button("✂️ Efficient Mode", use_container_width=True):
        st.session_state.rev_change = 0
...
insight_type, insight_text = runway_insight(runway_val, rev_change, burn_change)
st.markdown(
...
with k3:
    ...
    kpi_card(net_label, fmt_currency(net),
...
with chart_col2:
    ...
    if is_scenario:
        fig.add_trace(go.Scatter(
            x=month_labels, y=base_df["burn"], name="Baseline",
            mode="lines", line=dict(color=COLORS["baseline"], width=1.5, dash="dot"),
            hovertemplate="Baseline: $%{y:,.0f}<extra></extra>",
        ))
    fig.add_trace(go.Scatter(
        x=month_labels, y=scenario_df["burn"], name="Burn",
...
if is_scenario:
    fig.add_trace(go.Scatter(
        x=month_labels, y=base_df["cash_on_hand"], name="Baseline Cash",
        mode="lines", line=dict(color=COLORS["baseline"], width=1.5, dash="dot"),
        hovertemplate="Baseline: $%{y:,.0f}<extra></extra>",
    ))
fig.add_trace(go.Scatter(
    x=month_labels, y=scenario_df["cash_on_hand"], name="Cash on Hand",
...
with chart_col3:
    ...
    fig.add_trace(go.Bar(
        x=month_labels, y=runway_vals, name="Runway (months)",
